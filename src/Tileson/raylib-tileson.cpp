...
{
    return ((RaylibTilesonData*)map.data)->map->getLayers();
}

tson::Layer* GetLayerFromMap(Map &map, std::string layerName) 
{
    return ((RaylibTilesonData*)map.data)->map->getLayer(layerName);
}

Rectangle LayerObjectRectToRaylibRect(tson::Object rect) 
{
    return Rectangle{rect.getPosition().x, rect.getPosition().y, rect.getSize().x, rect.getSize().y};
}

Rectangle GetMapBounds(Map &map) 
{
    int mapwidth = ((RaylibTilesonData*)map.data)->map->getTileSize().x * ((RaylibTilesonData*)map.data)->map->getSize().x;
    int mapheight = ((RaylibTilesonData*)map.data)->map->getTileSize().y * ((RaylibTilesonData*)map.data)->map->getSize().y;
    return Rectangle{0, 0, mapwidth, mapheight};
}
